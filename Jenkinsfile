                ...
                script {
                    if (params.DEPLOY_MODE == 'local') {
                        sh '''
                            ...
                            cp -f docker/nginx/conf/nginx.conf "${PUBLIC_NGINX_DIR}/nginx.conf"
                            chmod +x "${DEPLOY_SCRIPT_DIR}/deploy-backend-host.sh"
                            PACKAGE_DIR="${DEPLOY_DIR}/packages" LOG_DIR="${LOG_DIR}" PUBLIC_NGINX_CONF_SOURCE="${PUBLIC_NGINX_DIR}/nginx.conf" PUBLIC_NGINX_CONF_TARGET="${PUBLIC_NGINX_CONF_TARGET}" POST_DEPLOY_CMD="${POST_DEPLOY_CMD}" "${DEPLOY_SCRIPT_DIR}/deploy-backend-host.sh"
                        '''
                    } else {
                        sshagent(credentials: [params.SSH_CREDENTIALS_ID]) {
                            sh '''
                                ...
                                scp -o StrictHostKeyChecking=no docker/nginx/conf/nginx.conf ${SSH_USER}@${SSH_HOST}:${PUBLIC_NGINX_DIR}/nginx.conf
                                ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SSH_HOST} "chmod +x ${DEPLOY_SCRIPT_DIR}/deploy-backend-host.sh && PACKAGE_DIR=${DEPLOY_DIR}/packages LOG_DIR=${LOG_DIR} PUBLIC_NGINX_CONF_SOURCE=${PUBLIC_NGINX_DIR}/nginx.conf PUBLIC_NGINX_CONF_TARGET=${PUBLIC_NGINX_CONF_TARGET} POST_DEPLOY_CMD='${POST_DEPLOY_CMD}' ${DEPLOY_SCRIPT_DIR}/deploy-backend-host.sh"
                            '''
                        }
                    }
                }
